...
def text_similarity(feat_path, qid1, qid2):

    q_feat1 = torch.tensor(np.load(f"{feat_path}/qid{qid1}.npz")['last_hidden_state'].astype(np.float32))
    q_feat2 = torch.tensor(np.load(f"{feat_path}/qid{qid2}.npz")['last_hidden_state'].astype(np.float32))

    q_feat1 /= (q_feat1).norm(dim=-1, keepdim=True)
    q_feat2 /= (q_feat2).norm(dim=-1, keepdim=True)
    
    similarity = (100.0 * q_feat1 @ q_feat2.T)
...
for data in datalist:
    ...
    # crop augmentation
    new_data = feat_mix(data, moments=moments, non_moments=non_moments, ctx_l=ctx_l, clip_len=clip_len, db_range=db_range, moment_db=moment_db, feat_path=feat_path)
    if new_data:
